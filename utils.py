...
def build_word_vector_matrix(vector_file, n_words):
        ...
        with codecs.open(vector_file, 'r', 'utf-8') as f:
                for i, line in enumerate(f):
                        ...
                        vec_list = [float(j) for j in sr[1:]]
                        np_arrays.append(np.array(vec_list))
                        word_to_vector_dict[sr[0]] = vec_list
                        if i == n_words - 1:
                                return np.array(np_arrays), labels_array, word_to_vector_dict
                return np.array(np_arrays), labels_array, word_to_vector_dict
